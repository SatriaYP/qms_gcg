    ...
    <script>
        ...
        document.getElementById("scan-input").addEventListener("input", function () {
            ...
            if (inputField.value.length === 5) {
                ...
                const spesialis_temp = <?php echo json_encode($spesialis_temp); ?>;
                // Kirim data ke server menggunakan AJAX
                fetch('/antrian/queue_dokter/<?php echo $spesialis_temp; ?>', {
    method: 'POST',
    headers: {
        'Content-Type': 'application/json',
        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
    },
    body: JSON.stringify({ queue_number_dokter: queueNumber, spesialis: spesialis_temp })
})
